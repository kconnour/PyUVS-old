...
from pyuvs.aux.arrays import FUVSensitivity, MUVSensitivityUpdated, MUVSensitivityPipeline, NightsideMUVTemplates, _AuxiliaryDict
from pyuvs.files import DataFilename

from astropy.io import fits

orbit_number = 5738
files = sorted(glob.glob(f'/Volumes/MAVEN Data/iuvs_data/level1b/orbit05700/*l1b_apoapse-orbit{orbit_number:0>5}-muv*.fits.gz'))
...
class _InstrumentSettings:
    ...
    def __get_settings(self) -> dict:
        return {
            ...
            'spectral_pixel_start': self.__contents['primary'].header['spe_ofs'],
            ...
            'wavelength_width': np.median(self.__contents['observation'].data['wavelength_width'][0])
        }
...
class CalibrationCurve:
    ...
    def __calculate_calibration_curve(self):
        ...
        bin_omega = pixel_omega * self.__instrument_settings['spatial_bin_width']
        line_effective_area = np.array([np.interp(i, self.__wavelength, self.__sensitivity) for i in self.__instrument_settings['wavelengths']])
        return self.__instrument_settings['wavelength_width'] * self.__instrument_settings['detector_gain'] * self.__instrument_settings['integration_time'] * kR * line_effective_area * bin_omega
...
class MLRFitter:
    ...
    def __init__(self, spectrum, spectrum_uncertainty, templates,
        ...
        self.__template_names = self.__add_constant_to_template_names(template_names)
    ...
    def get_integrated_intensity(self, calibration_curve, wavelength_width):
        return np.array([np.sum(self.fit_coefficients[i] * self.__templates[i] / calibration_curve * wavelength_width)
                         for i in range(1, len(self.__templates))])

    def get_integrated_intensity_uncertainty(self, calibration_curve, wavelength_width):
        return np.array([np.sum(self.fit_uncertainties[i] * self.__templates[i] / calibration_curve * wavelength_width)
                         for i in range(1, len(self.__templates))])

    def get_integrated_intensity_and_uncertainty(self, calibration_curve, wavelength_width):
        return self.get_integrated_intensity(calibration_curve, wavelength_width), self.get_integrated_intensity_uncertainty(calibration_curve, wavelength_width)
...
class PipelineMLR:
    ...
    def __raise_value_error_if_invalid_binning_table(self):
        if self.__instrument_settings['spectral_pixel_start'] % self.__instrument_settings['spectral_bin_width'] != 0:
            raise ValueError('Invalid binning table. Contact Justin Deighan for assistance: justin.deighan@lasp.colorado.edu.')

    def __calculate_saturation_threshold(self):
        return cmos_pixel_well_depth * self.__instrument_settings['spatial_bin_width'] * self.__instrument_settings['spectral_bin_width']

    def __load_sensitivity_curve(self):
        return MUVSensitivityPipeline() if self.__instrument_settings['channel'] == 'muv' else FUVSensitivity()

    def __calculate_calibration_curve(self):
        return CalibrationCurve(self.__instrument_settings, self.__sensitivity_curve).detector_calibration
    ...
    def __load_nightside_templates(self):
        ...
        n_spectral_bins = int(1024 / self.__instrument_settings['spectral_bin_width'])
        pixel_start = int(self.__instrument_settings['spectral_pixel_start']/self.__instrument_settings['spectral_bin_width'])
        if n_spectral_bins != 1024:
            return list(templates.keys()), np.array([self.__rebin_factor(templates[i]) for i in list(templates.keys())])[:, pixel_start:pixel_start+l1b.n_wavelengths]
        else:
            return list(templates.keys()), np.array([templates[i] for i in list(templates.keys())])[:, pixel_start:pixel_start+l1b.n_wavelengths]

    def __rebin_factor(self, array: np.ndarray):
        new_spectrum = np.array([np.sum(array[i:i+self.__instrument_settings['spectral_bin_width']]) for i in range(0, len(array)-self.__instrument_settings['spectral_bin_width'], self.__instrument_settings['spectral_bin_width'])])
        new_spectrum /= np.sum(new_spectrum * self.__instrument_settings['wavelength_width'])
        return new_spectrum
    
    def __perform_pixel_by_pixel_mlr_fit(self):
        template_names, templates = self.__load_nightside_templates()
        fit_array = np.zeros((self.__l1b.n_integrations, self.__l1b.n_positions), dtype=object)
        radiance_array = np.zeros((self.__l1b.n_integrations, self.__l1b.n_positions, len(template_names)))
        uncertainty_array = np.zeros((self.__l1b.n_integrations, self.__l1b.n_positions, len(template_names)))
        for integration in range(self.__l1b.n_integrations):
            for spatial_bin in range(self.__l1b.n_positions):
                spectrum = self.__l1b['detector_dark_subtracted'].data[integration, spatial_bin]
                spectrum_uncertainty = self.__l1b['random_dn_unc'].data[integration, spatial_bin]
                fit = MLRFitter(spectrum, spectrum_uncertainty, templates, template_names)
                fit_array[integration, spatial_bin] = fit
                radiance_array[integration, spatial_bin] = fit.get_integrated_intensity(self.__detector_calibration[spatial_bin], self.__instrument_settings['spectral_bin_width'])
                uncertainty_array[integration, spatial_bin] = fit.get_integrated_intensity_uncertainty(self.__detector_calibration[spatial_bin], self.__instrument_settings['spectral_bin_width'])
        return fit_array, radiance_array, uncertainty_array, np.array(template_names)
